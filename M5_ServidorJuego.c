...
int MostrarListaConectados (char respuesta6[512], char conectado6[300])
{
    ...
    for(i=0; i<miLista.num; i++)
    {
   	 if  (miLista.conectados[i].socket == 4 && miLista.conectados[i].socket == 5)
   	 {
   		 ...
   		 sprintf(conectado6, "6/%s*%d-%s", conectado6, miLista.conectados[i].socket , miLista.conectados[i].nombre);
   	 }
...
void Notificacion(char respuestaN[512])
{
    char notificacion[20];
    sprintf(notificacion,"6/%s",respuestaN);
/*    int j;*/
    //i es el índice de numero de clientes conectados
/*    for (j=0;j<i;j++)*/
/*    {*/
